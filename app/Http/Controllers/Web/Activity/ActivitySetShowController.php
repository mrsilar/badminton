...
class ActivitySetShowController extends WebController
{
	...
	public function groupinfo(Request $request)
	{
		...
		if ($activity_turn->turn == 2 && $activity_turn->game_system == 1) {
			...
			$team_group_tmp=[];
			foreach ($out_team_group['data'] as $key => $value) {
				$team_group_tmp[$value->group_group_num][$value->group_num][]=$value;
			}

			$out_group_team_match_match=[];
			...
			foreach ($out_group_match['data']  as $key => $value) {
				...
				$out_group_team_match_match[]=$value;
			}
			$out_group_match=tran_key($out_group_match['data'],'group_group_num');

			Template::assign('turn', $request->input('turn', 1));
			Template::assign('team_match', $team_group_tmp);
			Template::assign('group_match', $out_group_match);
		...
		$team_match=[];
		foreach ($out_team_match['data'] as $key => $value) {
			# code...team_group_num
			$team_match[$value->team_group_num][]=$value;
		}
	...
	public function resultinfo(Request $request)
	{
		...
		//第二轮而且外循环
		if ($activity_turn->turn == 2 && $activity_turn->game_system == 1) {die('dd');
			$out_group_match = ActivitySetShowModel::group_match($request->all());
