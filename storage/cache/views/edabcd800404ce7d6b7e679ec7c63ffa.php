...
<header data-am-widget="header"
    ...
    <h1 align="center" class="am-header-title"> 第<?php echo isset($turn) ? $turn : ''; ?>轮比赛排名 </h1>
</header>
<div>
    <?php foreach($rank as $key => $row) { ?>
    <h1>第<?php echo isset($key) ? $key : ''; ?>大组(第<?php echo isset($row['from']) ? $row['from'] : ''; ?>~<?php echo isset($row['to']) ? $row['to'] : ''; ?>名)</h1>
    ...
    <table class="am-table">
        ...
        <tr>
            ...
            <td>
                <?php if ($rank_is == 1) { ?>
                <input value="<?php echo $r->rank==10000?'':$r->rank; ?>" class="rankclass">
                <input type="hidden" value="<?php echo isset($r->id) ? $r->id : ''; ?>" class="idclass">
                <?php } else { ?>
                <?php echo $r->rank==10000?'':$r->rank; ?>
                <?php } ?>
            </td>
            <?php if ($rank_is==1) { ?>
            <td>
    ...
    <a type="button" class="am-btn am-btn-warning"
       href="/h5/member/activity/set/rank?activity_turn_id=<?php echo isset($activity_turn_id) ? $activity_turn_id : ''; ?>&rank=0">自动排名</a>

    <a type="button" class="am-btn am-btn-warning"
       href="/h5/member/activity/set/rank?activity_turn_id=<?php echo isset($activity_turn_id) ? $activity_turn_id : ''; ?>&rank=1">手动排名</a>
</div>
